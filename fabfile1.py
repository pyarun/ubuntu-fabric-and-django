...
def check_project_name(name):
    ...
    if os.path.exists(os.path.join(env.VIRTUALENV_PATH, name)):
        message = 'VIRTUALENV with such name already exists at {}'.format(env.VIRTUALENV_PATH)
        feedback = prompt("Do you want to use the existing env(y -> yes)?")
        if feedback!='y':
            return False, message
        
    if os.path.exists(os.path.join(env.PROJECT_DIR, name)):
        message = ('Project with such name already exists at {}'.format(env.PROJECT_DIR))
        return False, message
...
def startproject(name):
    ...
    ve_path = os.path.abspath(os.path.join(env.VIRTUALENV_PATH, name))
    
    project_path = os.path.abspath(os.path.join(env.PROJECT_DIR, name))
    source_path = os.path.abspath(os.path.join(project_path, SOURCE_DIRECTORY_NAME))
    
    local('mkdir %s' % project_path)
    local('mkdir %s' % source_path)
    
    with lcd(source_path):
        with prefix('. %s' % ve_activate_prefix(ve_path)):
            ...
            create_uwsgi_files(name, project_path)
